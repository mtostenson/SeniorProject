...
public class WavRecorder implements OnRecordPositionUpdateListener {
      ...
      try {
         if (state == State.INITIALIZING) {
            if (audioRecord.getState() == AudioRecord.STATE_INITIALIZED) {
               ...
               // fileForward = new File(context.getFilesDir(), "forward.wav");
               fileForward = new File(folder, "EXT_TEST_FORWARD.wav");
               
               // fileReverse = new File(context.getFilesDir(), "reverse.wav");
   ...
   public void createWavHeader(RandomAccessFile writer) throws IOException {
      ...
      writer.writeInt(Integer
            .reverseBytes(SAMPLE_RATE * SAMPLES * CHANNELS / 8));
   ...
   public void reverse() throws IOException {
      int SAMPLE_SIZE = CHANNELS * SAMPLES / 8;
      long locater = writerForward.length() - SAMPLE_SIZE;
